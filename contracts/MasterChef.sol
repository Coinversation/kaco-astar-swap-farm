...
contract MasterChef is Ownable {
    ...
    uint256 public allocOther;
    uint256 public allocAstar;
    uint256 public kacPerAstarBlock;
    ...
    constructor(
        ...
        allocOther = _allocOther;
        allocAstar = _allocAstar;

        updateKacPerAstarBlock(_kacPerBlock, _allocOther, _allocAstar, false);
    ...
    function updateKacPerBlock(
        ...
        updateKacPerAstarBlock(_kacPerBlock, allocOther, allocAstar, _withUpdate);
        kacPerBlock = _kacPerBlock;
    }

    function updateAllocOther(
        uint256 _allocOther,
        bool _withUpdate
    ) external onlyOwner {
        updateKacPerAstarBlock(kacPerBlock, _allocOther, allocAstar, _withUpdate);
        allocOther = _allocOther;
    }

    function updateAllocAstar(
        uint256 _allocAstar,
        bool _withUpdate
    ) external onlyOwner {
        updateKacPerAstarBlock(kacPerBlock, allocOther, _allocAstar, _withUpdate);
        allocAstar = _allocAstar;
    }

    function updateKacPerAstarBlock(
        uint256 _kacPerBlock,
        uint256 _allocOther,
        uint256 _allocAstar,
        bool _withUpdate
    ) internal {
        kacPerAstarBlock = (_allocAstar * 1e12 / (_allocOther + _allocAstar) * _kacPerBlock * 4) / 1e12;
        if (_withUpdate) {
            massUpdatePools();
        }
    }
    ...
    function pendingCake(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 kacReward = multiplier.mul(kacPerAstarBlock).mul(pool.allocPoint).div(totalAllocPoint);
